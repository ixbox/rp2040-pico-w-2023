...
USB-Cポートから直接PCに接続すると、デバイスが認識されない場合がある。
USB Type-A（USB 2.0）ハブやアダプタを経由して接続すると正常に認識される。

**推定原因**: USB-CのCC（Configuration Channel）ピンに必要な5.1kΩプルダウン抵抗が未実装。
オリジナルのmicro-B設計をそのままUSB-Cコネクタに載せ替えただけと思われる。
USB-Cではホスト側がCCピンのプルダウンを検出してデバイスの存在や向きを判定するため、
この抵抗がないとUSB-C to USB-Cケーブルでは接続が成立しない。
Type-Aハブ/アダプタ経由ではCCネゴシエーションが不要なため問題なく動作する。

> **ハマりポイント**: BOOTSELモードでもATコマンド通信でも同様。USB-C直結で認識しない場合は
> Type-A変換を試すこと。
...
1. **HTTPヘッダバッファ固定長** (`ESP8285Wifi.cpp`)
   - `header[384]` 固定 — method, path, host, Content-Type が長い場合に切り詰めが発生する
   - body は `_serial.print()` で別途送信するため、body 側のサイズ制限はない
